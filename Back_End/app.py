...
@app.route('/get-all-user/<offset>/<limit>', methods = ['GET'])
def get_all_user(offset, limit):
  res = service.get_all_user(int(offset), int(limit))
  if res:
    return jsonify({
      'success': True,
      **res
...
def register():
  ...
  res = service.create_user(email, password, firstname, lastname, phone, address, role)
  if res:
    return jsonify({
      'success': 'true'
    }) , 200
...
def update_info():
  ...
  res = service.update_info(user_id, firstname, lastname, phone, address, role)
  
  if res:
    return jsonify({'success': True}) , 200
  else: 
    return jsonify({'success': False, 'error': 'LACK USER INFO'}), 201
...
@app.route('/get-product-types/<category>')
def get_product_type(category):
  found_products = service.get_all_product_types(category)
  return jsonify({
    "types": found_products
  }), 200
...
@app.route('/get-product-by-id/<id>')
def get_product_by_id(id):
  found_product = service.get_product_by_id(id)
...
@app.route('/get-all-product/<offset>/<limit>', methods = ['GET'])
def get_all_product(offset, limit):
  res = service.get_all_product(int(offset), int(limit))
  if res:
    return jsonify({
      'success': True,
      **res
